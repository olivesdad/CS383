...
tests_positive[has_measles==True] = np.random.choice([True,False], size=tests_positive[has_measles==True].size, p=[p_pos_if_measles, 1-p_pos_if_measles])
...
tests_positive[has_measles == False] = np.random.choice([True, False], size=tests_positive[has_measles==False].size, p=[p_pos_if_okay, 1-p_pos_if_okay ])
...
has_measles[has_measles & tests_positive].sum()/tests_positive.sum()
...
# (expression)
# Variables to be used: has_measles, tests_positive 
((has_measles == False) & (tests_positive == False)).sum() / (tests_positive==False).sum()
...
# (expression)
# Variables to be used: has_measles, tests_positive 
((has_measles==True) & (tests_positive==False) ).sum() / (tests_positive==False).sum()
