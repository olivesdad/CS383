...
# assume unemp is defined
unemp[unemp<unemp['COMPUTER SCIENCE']].size/unemp.size
...
(df['Women']/df['Total']).sort_values(ascending=False).keys()[0]
...
df.loc[(df['Women']/df['Total'])[lambda x: x > .90].keys()]['Median']
...
df.loc[(df['Women']/df['Total'])[lambda x: x < .15].keys()]['Median']
...
df.loc[df['Median' ].sort_values(ascending=False)[0:10].keys()][['Median', 'P25th', 'P75th']]
...
df.loc[df['Median' ].sort_values(ascending=True)[0:10].keys()][['Median', 'P25th', 'P75th']]
...
(df['Non_college_jobs']/df['Total']).sort_values(ascending=False)[0:10]
...
df['HighShareWomen'] = (df['Women']/df['Total'] > 0.5).values
...
df['HighShareWomen'].sum()/df.shape[0]
...
(df['Low_wage_jobs']/df['Total']).sort_values(ascending=False)[0:10]
...
df['Major_share'] = (df['Total'] / df['Total'].sum()).tolist()
...
""" 
problem 1.28: execution error: unexpected EOF while parsing (<string>, line 1)
problem 1.29: execution error: unexpected EOF while parsing (<string>, line 1)
""" 